...
type Result = {
  ...
  channels: Array<{ id: string; title: string | null; is_public: boolean }>;
};
...
export default function SearchPage() {
  ...
  const [res, setRes] = useState<Result>({ users: [], messages: [], channels: [] });
  ...
  async function joinChannel(id: string) {
    const { error } = await createClient().rpc('join_public_channel', { conv: id });
    if (!error) {
      await load();
      router.push(`/c/${id}`);
    }
  ...
  return (
    <AppShell sidebar={<ChatList />}>
      <div className="mx-auto max-w-lg px-4 py-8">
        ...
        <h2 className="mt-6 font-bold">کانال‌های عمومی</h2>
        {res.channels.map((c) => (
          <button
            key={c.id}
            className="block w-full py-2 text-right"
            onClick={() => void joinChannel(c.id)}
          >
            {c.title ?? 'کانال'} <span className="text-xs text-accent">عضویت</span>
          </button>
        ))}
